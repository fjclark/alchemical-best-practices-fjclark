...
\begin{frontmatter}
\maketitle
\todo[inline, color=green!20]{FC: @Volunteer update initial contributor list to reflect sections copied from part 1.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Abstract
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
...
\todo[inline, color=green!20]{FC: @Volunteer Update abstract to highlight the split of the document and focus on multiple calculations.}
...
\label{sec:intro}
\todo[inline, color=green!20]{FC: @Volunteer to add section briefly summarising the same section in the first paper, directing readers to the first paper, and highlighting their use a high-throughput, multiple calculation setting.}

\section{Prerequisites and Scope}
\label{sec:pre}

\todo[inline, color=green!20]{FC: @Volunteer to update clearly delineating the separation between the first (single calculation) and second (multiple caluclations) papers.}

\todo[inline, color=green!20]{FC: @Volunteer to a) integrate the sections cut directly from v1 of the paper and b) consider adding further sections as required.}
\section{How should alchemical simulations be applied to drug discovery?}
\label{sec:drugdiscovery}
\todo[inline, color=green!20]{FC: Is this section too broad? Should we split accross rest of paper?}
\todo[inline, color=green!20]{FC: Should we clearly seperate and discuss needs of lead opt. and virtual screening? For example recent work with fast ABFE from Sandbox AQ.}
...
\section{Which calculations should I run, and when?}
\label{sec:which_calculations}
\todo[inline, color=green!20]{FC: Possibly add discussion of methods which are not easily characterised as single/ multi e.g. lambda dynamics? Maybe a better distinction is calculations where we are interested in more than two end states.}
\todo[inline, color=green!20]{FC: @OpenFE team to add discussion of how to run and adaptively update perturbation networks.}
\todo[inline, color=green!20]{FC: Possibly add discussion of different protocols, especially active learning. Best practices may not be well established but many companies are running at scale.}
\todo[inline, color=green!20]{FC: Discussion of merits of ABFE/ RBFE for virtual screening? See @Gary Tresadern comment from first meeting - lots of discussion around ABFE calculations for virtual screening.}

\paragraph{Perturbation maps}
\todo[inline, color=green!20]{FC: @Jenke Scheen/ Mary Pitman/ OpenFE team to update discussion of perturbation maps?}
...
\section{How should I automate my calculations within my workflow?}
\label{sec:automation}
\todo[inline, color=green!20]{FC: Include e.g. automated selection of restraints for ABFE, automated selection of where to concentrate sampling time e.g. paper from Huafeng Xu, discussion of available tools?}
\todo[inline, color=green!20]{FC: @OpenFE team well suited. Discuss tools like alchemiscale? Discuss e.g. integration with REINVENT/ Maize for active learning?}
...
\todo[inline, color=green!20]{FC: @Peter Coveney/ @Agastya P Bhati to cover uncertainty estimation for multiple simulations. Move cycle closure etc sections here.}
...
\label{sec:plot_data}
\todo[inline, color=green!20]{FC: @JS/ OpenFE people to contribute?} 
\todo[inline, color=green!20]{FC: @JS / OpenFE team/ Volunteers to expand on utility/ limitations of common metrics. Also see @Michael Gilson's GitHub issue.}
...
\section{Checklist}
\label{sec:checklist}
\todo[inline, color=green!20]{FC: @Volunteer to update checklist once manuscript is nearing completion.}
\begin{Checklists*}
