...
\todo[inline, color=green!20]{FC: @Volunteer Update abstract to highlight the split of the document and focus on the single calculations.}
...
\label{sec:intro}
\todo[inline, color=green!20]{FC: No major changes.}
...
\begin{figure}
    ...
    \todo[inline, color=green!20]{FC: @JS to think about how this Figure can be used as the "central axis for the paper".}
...
\todo[inline, color=green!20]{FC: @Volunteer to update reflecting new split between single and multiple calculations. @MRS to update best practices on multiple binding sites.}
\todo[inline, color=green!20]{FC: @Everyone to contribute to section on "unsettled practices"/ "current limitations", as suggested by JDC. Either here or in entirely new section.}
...
\label{sec:theory}
\todo[inline, color=green!20]{FC: @EG to update theory. Will frame it to be as general as possible, while remaining accessible to beginners. Will align with Gilson et al. 1997 paper.}
...
\todo[inline, color=green!20]{FC: @ASJSM suggestion: Possibly a place to discuss things which are possible but aren't in scope. Might also be a place to mention outstanding challenges.}
...
\todo[inline, color=green!20]{FC: There was substantial interest in expanding this (see initial suggestions/ discussion).}
\todo[inline, color=green!20]{FC: @Irfan Alibay/ @Volunteer to suggest additional points}
\todo[inline, color=green!20]{FC: @Irfan Alibay/ @Emilio Gallicchio/ @Volunteer to expand on approaches for explicitly accounting for protonation/ tautomer changes.}
\todo[inline, color=green!20]{FC: @Hannah Bruce Macdonald/ @David Slowchower/ @Volunteer to expand range of systems discussed, e.g. membrane proteins, cofactors, multimers, PPIs.}
...
\todo[inline, color=green!20]{FC: Move to second paper if we are assuming a single calculation?}
...
\todo[inline, color=green!20]{FC: @Volunteer: Expand coverage of free energies of hydration/ partition coefficients (assuming that we want to make coverage of applications equal).}
...
\todo[inline, color=green!20]{FC: @ASJSM suggestion - possibily expand to include other methods group agrees on.}
\todo[inline, color=green!20]{FC: @Sheenam Khuttan / @Emilio Gallicchio to contribute section on ATM.}
...
\todo[inline, color=green!20]{FC: @Stefan Boresch suggestion: Remove single/ dual topology section and begin with a more fundamental discussion which might more naturally include all options e.g. ATM. This should follow @EG's changes to make the theory section more general.}
...
\todo[inline, color=green!20]{FC: @Volunteer: Mainly move to second paper and assume that change is small (and hence atom mapping is trivial). @Benjamin Ries to discuss 3D atom mapping in second paper?}
...
\todo[inline, color=green!20]{FC: @EG to detail possible issues with theoretical discussion. See PR from @FC to improve discussion of restraint schemes and automated restraint selection. @Stefan Boresch may like to weigh in on analytical corrections/ general discussion. @AR to review?}
...
\todo[inline, color=green!20]{FC: Best practices have evolved. @Vytas Gapsys/ @ Chris Oostenbrink to update.}
...
\todo[inline, color=green!20]{FC: @FC Move parts of this to analysis section (in Single/ Multiple papers).}
...
\todo[inline, color=green!20]{FC: @MRS to update discussion of treatment of multiple binding poses}
...
\todo[inline, color=green!20]{FC: Short discussion of recommended integrators? BAOAB?}
...
\todo[inline, color=green!20]{FC: Can section on decorrelation/ equilibration detection be shortened?}
...
\todo[inline, color=green!20]{FC: Is this actually a good idea?}
...
\todo[inline, color=green!20]{FC: To be split between single/ multiple papers. @Agastya P Bhati/ Peter Coveney to add to multiple repeat uncertainty quantification? Cycle closure etc. to be moved to second paper.}
\todo[inline, color=green!20]{FC: Add discussion of bootstrapping vs t-based confidence intervals for repeats of single calculations? Both poor}
...
\todo[inline, color=green!20]{FC: @Volunteer to update.}
...
\section{Checklist}
\label{sec:checklist}
\todo[inline, color=green!20]{FC: @Volunteer to update checklists/ move relevant sections to second paper once near completion.}
\begin{Checklists*}
